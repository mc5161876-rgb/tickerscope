...
import { api, type FinancialsPayload, type Freq, type PriceRange, type SegmentsPayload, type TickerPayload } from "../lib/api";
...
const CHART_KEYS: ChartKey[] = ["price", "revenue", "ebitda", "segments", "ocf", "fcf", "capex", "netincome"];

/** MAR-56: the cash-flow / earnings cards, in the order they read best on the page. */
const CASH_KEYS = ["ocf", "capex", "fcf", "netincome"] as const;
type CashKey = (typeof CASH_KEYS)[number];
const CASH_SERIES: Record<CashKey, keyof Pick<FinancialsPayload, "operating_cash_flow" | "capital_expenditure" | "free_cash_flow" | "net_income">> = {
  ocf: "operating_cash_flow",
  capex: "capital_expenditure",
  fcf: "free_cash_flow",
  netincome: "net_income",
};
const CASH_TITLE: Record<CashKey, string> = {
...
export function TickerPage({
  ...
  const byPeriod = freq === "quarterly" ? "by quarter" : "by fiscal year";
  const chartSubtitle: Record<ChartKey, string | null> = {
    ...
    ocf: `Cash generated by the business, ${byPeriod}`,
    // capex is a negative outflow in the source; the card shows cash spent, so say so
    capex: `Cash spent on property, plant & equipment, ${byPeriod}`,
    fcf:
      f.data?.free_cash_flow_method === "calculated"
        ? "calculated: operating cash flow − capital expenditure"
        : `What's left after running and maintaining the business, ${byPeriod}`,
    netincome: `Profit after every cost and tax, ${byPeriod}`,
  };
  const hasChart: Record<ChartKey, boolean> = {
    ...
    ocf: !!f.data?.operating_cash_flow.length,
    capex: !!f.data?.capital_expenditure.length,
    fcf: !!f.data?.free_cash_flow.length,
    netincome: !!f.data?.net_income.length,
  };
  const renderChart = (k: ChartKey, opts: { detail?: boolean; hideTooltip?: boolean; withLegend?: boolean } = {}): ReactNode => {
    switch (k) {
      ...
      default:
        return f.data ? (
          <BarSeriesChart points={f.data[CASH_SERIES[k as CashKey]]} name={CASH_TITLE[k as CashKey]} detail={opts.detail} hideTooltip={opts.hideTooltip} />
        ) : null;
    }
  };
    ...
    const pts =
      k === "revenue"
        ...
        : k === "ebitda"
          ...
          : k === "segments"
            ? sg.data?.periods
            : f.data?.[CASH_SERIES[k as CashKey]];
  ...
  const chartSource = (k: ChartKey): string => {
    ...
    if (k !== "revenue" && k !== "ebitda") return "Source: yfinance (Yahoo Finance)";
  ...
  return (
    <ExplainerProvider>
          ...
          {loadingHead ? (
            ...
            <>
              <div className="ticker-title">
                <CompanyLogo ticker={ticker} name={companyName} size={40} />
      ...
      {/* ------------------------------------------------ cash flow & earnings (MAR-56) */}
      <section className="section" aria-label="Cash flow and earnings">
        <SectionHead
          title="Cash flow & earnings"
          right={
            <Segmented ariaLabel="Financials period" options={FREQ_OPTIONS} value={freqUi} onChange={setFreqUi} />
          }
        />
        <div className="chart-grid">
          {CASH_KEYS.map((k) => (
            <ChartCard
              key={k}
              title={chartTitle[k]}
              subtitle={chartSubtitle[k]}
              onExpand={hasChart[k] ? () => openFullscreen(k) : undefined}
              onSave={hasChart[k] ? () => void saveImage(k) : undefined}
              saving={exporting}
            >
              {f.status === "loading" && !f.data ? (
                <ChartSkeleton />
